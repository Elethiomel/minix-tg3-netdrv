...
PRIVATE int tg3_start_transmit(message * mp, struct tg3 * tp)
{
	...
	r = sys_safecopyfrom(mp->DL_ENDPT,
			     mp->DL_GRANT,
...
PRIVATE void upload_packet(tg3_t * tp, message * mp, iovec_t * iovec, int len)
{
	...
	if (r != OK) {
		printf("Client was %d\nGrant was %d\nCount was %d\n", 
				mp->DL_ENDPT, mp->DL_GRANT, mp->DL_COUNT);
...
{
	munmap(tp->rx_std_buffers, (sizeof(struct ring_info) *
				    (TG3_RX_RING_SIZE +
				     TG3_RX_JUMBO_RING_SIZE)) +
	...
	if (tp->rx_std) {
		munmap(tp->rx_std, TG3_RX_RING_BYTES);
		tp->rx_std = NULL;
	}
	if (tp->rx_jumbo) {
		munmap(tp->rx_jumbo, TG3_RX_JUMBO_RING_BYTES);
		tp->rx_jumbo = NULL;
	}
	if (tp->rx_rcb) {
		munmap(tp->rx_rcb, TG3_RX_RCB_RING_BYTES(tp));
		tp->rx_rcb = NULL;
	}
	if (tp->tx_ring) {
		munmap(tp->tx_ring, TG3_TX_RING_BYTES);
		tp->tx_ring = NULL;
	}
	if (tp->hw_status) {
		munmap(tp->hw_status, TG3_HW_STATUS_SIZE);
		tp->hw_status = NULL;
	}
	if (tp->hw_stats) {
		munmap(tp->hw_stats, sizeof(struct tg3_hw_stats));
		tp->hw_stats = NULL;
	}
}
